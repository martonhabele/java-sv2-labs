...
class CafeTest {
    ...
    void testGetTotalIncome() {
        ...
        assertEquals(16_400, cafe.getTotalIncome());
    }

    @Test
    void testGetIncomeForOneDay() {
        Cafe cafe = new Cafe(orders);
        LocalDate date = LocalDate.of(2018, 5, 4);
        assertEquals(2600, cafe.getTotalIncome(date));
    }
    ...
    @Test
    void testGetFirstFiveOrders() {
        Cafe cafe = new Cafe(orders);
        LocalDate date = LocalDate.of(2018, 5, 3);
        LocalTime time = LocalTime.of(9, 19);
        List<CoffeeOrder> firstFive = cafe.getFirstFiveOrder(date);
        assertEquals(5, firstFive.size());
        assertEquals(LocalDateTime.of(date, time), firstFive.get(0).getOrderTime());
        assertEquals(2, firstFive.get(0).getCoffees().size());
    }
}