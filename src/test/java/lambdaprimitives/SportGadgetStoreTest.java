...
class SportGadgetStoreTest {
    ...
    void testGetAveragePriceForEmptyList() {
        SportGadgetStore store = new SportGadgetStore(Collections.emptyList());
        assertEquals(0, store.getAveragePrice());
    }
    ...
    void testGetExpensiveProductStatistics() {
        ...
        assertEquals(expectedString, store.getExpensiveProductStatistics(100d));
    }

    @Test
    void testGetExpensiveProductStatisticsForEmptyList() {
        SportGadgetStore store = new SportGadgetStore(Collections.emptyList());
        assertEquals("Nincs ilyen termék.", store.getExpensiveProductStatistics(100d));
    }

    @Test
    void testGetExpensiveProductStatisticsWithZeroProduct() {
        SportGadgetStore store = new SportGadgetStore(Arrays.asList(new Product("Tennis ball",
                2.47, 138)));
        assertEquals("Nincs ilyen termék.", store.getExpensiveProductStatistics(100d));
    }
}